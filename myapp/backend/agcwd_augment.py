...
# ================= AGCWD FUNCTION =================
def apply_agc_wd(image, alpha=0.8, brightness_gamma=0.9):
    """
    Adaptive Gamma Correction with Weighting Distribution (AGCWD)
    + slight brightness boost for ECG images
    """
    ...
    # Adaptive Gamma
    gamma = 1.0 - wd[process_chan]
    gamma = np.clip(gamma, 0.01, 10.0)

    # Apply AGCWD
    enhanced_float = np.power(img_float, gamma)

    # 🔆 Extra brightness boost (safe)
    enhanced_float = np.power(enhanced_float, brightness_gamma)
...
fig.suptitle("Original vs AGCWD + Brightness Boost ECG Images", fontsize=16)
...
for i, idx in enumerate(idxs):
    ...
    axes[i, 1].set_title("AGCWD + Brightness Boost")
    axes[i, 1].axis("off")
