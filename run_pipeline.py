...
def main():
    ...
    # 1. Run Perspective Correction (Homography)
    # Location: ecg_perspective/perspective_correction.py
    # Note: Reads from images folder, outputs to corrected_perspective
    perspective_script = os.path.join(base_dir, "ecg_perspective", "perspective_correction.py")
    run_script(perspective_script)

    # 2. Run Brightness Augmentation (CLAHE)
    # Location: ecg_brightness/clahe_augmentation.py
    # Note: Reads from corrected_perspective folder, outputs to augmented_clahe
    brightness_script = os.path.join(base_dir, "ecg_brightness", "clahe_augmentation.py")
    run_script(brightness_script)

    # 3. Run Contrast Augmentation (AGCWD)
